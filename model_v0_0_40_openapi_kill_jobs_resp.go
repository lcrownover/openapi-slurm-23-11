/*
Slurm REST API

API to access and control Slurm

API version: Slurm-24.05.3&openapi/slurmdbd&openapi/slurmctld
Contact: sales@schedmd.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_slurm_23_11

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the V0040OpenapiKillJobsResp type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &V0040OpenapiKillJobsResp{}

// V0040OpenapiKillJobsResp struct for V0040OpenapiKillJobsResp
type V0040OpenapiKillJobsResp struct {
	// List of jobs signal responses
	Status []V0040KillJobsRespJob `json:"status"`
	Meta *V0040OpenapiMeta `json:"meta,omitempty"`
	Errors []V0040OpenapiError `json:"errors,omitempty"`
	Warnings []V0040OpenapiWarning `json:"warnings,omitempty"`
}

type _V0040OpenapiKillJobsResp V0040OpenapiKillJobsResp

// NewV0040OpenapiKillJobsResp instantiates a new V0040OpenapiKillJobsResp object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewV0040OpenapiKillJobsResp(status []V0040KillJobsRespJob) *V0040OpenapiKillJobsResp {
	this := V0040OpenapiKillJobsResp{}
	this.Status = status
	return &this
}

// NewV0040OpenapiKillJobsRespWithDefaults instantiates a new V0040OpenapiKillJobsResp object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewV0040OpenapiKillJobsRespWithDefaults() *V0040OpenapiKillJobsResp {
	this := V0040OpenapiKillJobsResp{}
	return &this
}

// GetStatus returns the Status field value
func (o *V0040OpenapiKillJobsResp) GetStatus() []V0040KillJobsRespJob {
	if o == nil {
		var ret []V0040KillJobsRespJob
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *V0040OpenapiKillJobsResp) GetStatusOk() ([]V0040KillJobsRespJob, bool) {
	if o == nil {
		return nil, false
	}
	return o.Status, true
}

// SetStatus sets field value
func (o *V0040OpenapiKillJobsResp) SetStatus(v []V0040KillJobsRespJob) {
	o.Status = v
}

// GetMeta returns the Meta field value if set, zero value otherwise.
func (o *V0040OpenapiKillJobsResp) GetMeta() V0040OpenapiMeta {
	if o == nil || IsNil(o.Meta) {
		var ret V0040OpenapiMeta
		return ret
	}
	return *o.Meta
}

// GetMetaOk returns a tuple with the Meta field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V0040OpenapiKillJobsResp) GetMetaOk() (*V0040OpenapiMeta, bool) {
	if o == nil || IsNil(o.Meta) {
		return nil, false
	}
	return o.Meta, true
}

// HasMeta returns a boolean if a field has been set.
func (o *V0040OpenapiKillJobsResp) HasMeta() bool {
	if o != nil && !IsNil(o.Meta) {
		return true
	}

	return false
}

// SetMeta gets a reference to the given V0040OpenapiMeta and assigns it to the Meta field.
func (o *V0040OpenapiKillJobsResp) SetMeta(v V0040OpenapiMeta) {
	o.Meta = &v
}

// GetErrors returns the Errors field value if set, zero value otherwise.
func (o *V0040OpenapiKillJobsResp) GetErrors() []V0040OpenapiError {
	if o == nil || IsNil(o.Errors) {
		var ret []V0040OpenapiError
		return ret
	}
	return o.Errors
}

// GetErrorsOk returns a tuple with the Errors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V0040OpenapiKillJobsResp) GetErrorsOk() ([]V0040OpenapiError, bool) {
	if o == nil || IsNil(o.Errors) {
		return nil, false
	}
	return o.Errors, true
}

// HasErrors returns a boolean if a field has been set.
func (o *V0040OpenapiKillJobsResp) HasErrors() bool {
	if o != nil && !IsNil(o.Errors) {
		return true
	}

	return false
}

// SetErrors gets a reference to the given []V0040OpenapiError and assigns it to the Errors field.
func (o *V0040OpenapiKillJobsResp) SetErrors(v []V0040OpenapiError) {
	o.Errors = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *V0040OpenapiKillJobsResp) GetWarnings() []V0040OpenapiWarning {
	if o == nil || IsNil(o.Warnings) {
		var ret []V0040OpenapiWarning
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V0040OpenapiKillJobsResp) GetWarningsOk() ([]V0040OpenapiWarning, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *V0040OpenapiKillJobsResp) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []V0040OpenapiWarning and assigns it to the Warnings field.
func (o *V0040OpenapiKillJobsResp) SetWarnings(v []V0040OpenapiWarning) {
	o.Warnings = v
}

func (o V0040OpenapiKillJobsResp) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o V0040OpenapiKillJobsResp) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["status"] = o.Status
	if !IsNil(o.Meta) {
		toSerialize["meta"] = o.Meta
	}
	if !IsNil(o.Errors) {
		toSerialize["errors"] = o.Errors
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

func (o *V0040OpenapiKillJobsResp) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"status",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varV0040OpenapiKillJobsResp := _V0040OpenapiKillJobsResp{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varV0040OpenapiKillJobsResp)

	if err != nil {
		return err
	}

	*o = V0040OpenapiKillJobsResp(varV0040OpenapiKillJobsResp)

	return err
}

type NullableV0040OpenapiKillJobsResp struct {
	value *V0040OpenapiKillJobsResp
	isSet bool
}

func (v NullableV0040OpenapiKillJobsResp) Get() *V0040OpenapiKillJobsResp {
	return v.value
}

func (v *NullableV0040OpenapiKillJobsResp) Set(val *V0040OpenapiKillJobsResp) {
	v.value = val
	v.isSet = true
}

func (v NullableV0040OpenapiKillJobsResp) IsSet() bool {
	return v.isSet
}

func (v *NullableV0040OpenapiKillJobsResp) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableV0040OpenapiKillJobsResp(val *V0040OpenapiKillJobsResp) *NullableV0040OpenapiKillJobsResp {
	return &NullableV0040OpenapiKillJobsResp{value: val, isSet: true}
}

func (v NullableV0040OpenapiKillJobsResp) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableV0040OpenapiKillJobsResp) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


