# Go API client for openapi_slurm_23_11
...
```go
import openapi_slurm_23_11 "github.com/lcrownover/openapi-slurm-23-11"
```
...
For using other server than the one defined on index 0 set context value `openapi_slurm_23_11.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), openapi_slurm_23_11.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `openapi_slurm_23_11.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), openapi_slurm_23_11.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `openapi_slurm_23_11.ContextOperationServerIndices` and `openapi_slurm_23_11.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), openapi_slurm_23_11.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), openapi_slurm_23_11.ContextOperationServerVariables, map[string]map[string]string{
...
auth := context.WithValue(
		context.Background(),
		openapi_slurm_23_11.ContextAPIKeys,
		map[string]openapi_slurm_23_11.APIKey{
...
```go
auth := context.WithValue(context.Background(), openapi_slurm_23_11.ContextAccessToken, "BEARER_TOKEN_STRING")
r, err := client.Service.Operation(auth, args)
```
